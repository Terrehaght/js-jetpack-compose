...
(function() {
  ...
  const DIALOG_STYLES = `
    ...
    .md-dialog-actions { 
      ...
      flex-wrap: nowrap; /* Changed from wrap to nowrap for horizontal layout */
    }
    .md-dialog-btn {
    ...
    /* Only stack vertically on small screens or when buttons are too wide */
    ...
    /* Stack buttons when they overflow container */
  ...
  // ==========================================
  // Snackbar Styles with Theme Support
  // ==========================================
  const SNACKBAR_STYLES = `
    ...
    .msb-snackbar-container.msb-top { top: 16px; flex-direction: column-reverse; } /* Added top padding */
    ...
    /* Light mode default */
    :root:not(.dark) .msb-snackbar:not(.msb-error):not(.msb-success),
    :root.light .msb-snackbar:not(.msb-error):not(.msb-success) {
      background: #322F35;
      color: #F5EFF7;
    }
    
    /* Dark mode default */
    :root.dark .msb-snackbar:not(.msb-error):not(.msb-success) {
      background: #E6E1E5;
      color: #1C1B1F;
    }
    
    /* Error snackbar - Fixed transparency issue */
    .msb-snackbar.msb-error { 
      background: var(--md-error-container, #F9DEDC) !important; /* Added !important to override */
      color: var(--md-on-error-container, #410E0B) !important;
      opacity: 1 !important; /* Ensure full opacity */
    }
    
    /* Dark mode error */
    :root.dark .msb-snackbar.msb-error {
      background: #93000A !important;
      color: #FFDAD6 !important;
    }
    
    /* Success snackbar */
    .msb-snackbar.msb-success { 
    ...
    /* Dark mode success */
    :root.dark .msb-snackbar.msb-success {
    ...
    .msb-snackbar:not(.msb-error):not(.msb-success) .msb-snackbar-action {
      color: var(--md-inverse-primary, #D0BCFF);
    }
    :root.dark .msb-snackbar:not(.msb-error):not(.msb-success) .msb-snackbar-action {
      color: #6750A4;
    }
    .msb-snackbar:not(.msb-error):not(.msb-success) .msb-snackbar-action:hover { 
      background: rgba(208, 188, 255, 0.08); 
    }
    :root.dark .msb-snackbar:not(.msb-error):not(.msb-success) .msb-snackbar-action:hover { 
      background: rgba(103, 80, 164, 0.08); 
    }
    
    .msb-snackbar.msb-error .msb-snackbar-action { 
      color: var(--md-error, #B3261E); 
    }
    :root.dark .msb-snackbar.msb-error .msb-snackbar-action { 
      color: #FFB4AB; 
    }
    
    .msb-snackbar.msb-success .msb-snackbar-action { 
      color: #1B5E20; 
    }
    :root.dark .msb-snackbar.msb-success .msb-snackbar-action { 
      color: #C8E6C9; 
    }
    
    .msb-snackbar-close {
      background: transparent; border: none; color: inherit; opacity: 0.7;
      padding: 8px; margin-left: 4px; border-radius: 50%; 
      display: flex; align-items: center; justify-content: center;
  ...
  const MaterialDialog = (props = {}) => {
    ...
    const state = {
      ...
      // Smart coloring properties
      backgroundColor: props.backgroundColor || null, // e.g., '#808080' for gray
      lightenPercent: props.lightenPercent !== undefined ? props.lightenPercent : 80 // Default 80% lightening
    };
    ...
    const applySmartColoring = () => {
      if (!state.backgroundColor || !state.dialog) return;
      
      // Apply lightened color to dialog background only
      const lightenedColor = lightenColor(state.backgroundColor, state.lightenPercent);
      state.dialog.style.backgroundColor = lightenedColor;
      
      // Apply original (non-lightened) color to UI elements
      const originalColor = state.backgroundColor;
      
      // Apply to filled buttons
      const filledButtons = state.dialog.querySelectorAll('.md-dialog-btn.md-filled:not(.md-error)');
      filledButtons.forEach(btn => {
        btn.style.backgroundColor = originalColor;
        btn.style.color = '#ffffff'; // White text for contrast
      });
      
      // Apply to text buttons (non-filled)
      const textButtons = state.dialog.querySelectorAll('.md-dialog-btn:not(.md-filled):not(.md-error)');
      textButtons.forEach(btn => {
        btn.style.color = originalColor;
        // Hover effect with original color
        btn.addEventListener('mouseenter', function() {
          this.style.backgroundColor = originalColor + '14'; // ~8% opacity
        });
      ...
      // Apply to tonal buttons
      const tonalButtons = state.dialog.querySelectorAll('.md-dialog-btn.md-tonal:not(.md-error)');
      tonalButtons.forEach(btn => {
        btn.style.backgroundColor = lightenColor(originalColor, 90); // Very light version for tonal
        btn.style.color = originalColor;
      });
      
      // Apply to input border on focus
      if (state.inputElement) {
        state.inputElement.addEventListener('focus', function() {
          this.style.borderColor = originalColor;
        });
      }
      
      // Apply to icon color
      const iconElement = state.dialog.querySelector('.md-dialog-icon i');
      if (iconElement) {
    ...
    const build = () => {
      ...
      // Apply smart coloring after building
      applySmartColoring();
    };
    ...
    const instance = {
      ...
      setBackgroundColor(color, lightenPercent) {
        state.backgroundColor = color;
        if (lightenPercent !== undefined) state.lightenPercent = lightenPercent;
        return this;
      },
      setView(element) {
  ...
  MaterialDialog.alert = (title, message, options = {}) => {
    return MaterialDialog({ 
      ...
      lightenPercent: options.lightenPercent
    })
