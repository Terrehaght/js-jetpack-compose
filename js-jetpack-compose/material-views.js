...
const AndroidComponents = (() => {
 ...
 const TextView = (props = {}) => {
    const {
        ...
        seeMoreAlign = 'start',
        seeMoreColor = 'var(--md-primary, #007bff)',
        onExpand = null,
        onCollapse = null,
        onClick = null,
        // Icon properties
    ...
    if (borderRadius) container.style.borderRadius = safeDp(borderRadius);
    if (border) container.style.border = border;
    if (elevation > 0) {
        container.style.boxShadow = `0 ${elevation * 2}px ${elevation * 4}px rgba(0,0,0,${0.1 + elevation * 0.02})`;
    }

    // Apply icon gravity layout
    if (icon && (iconGravity === 'top' || iconGravity === 'bottom')) {
        container.style.flexDirection = 'column';
    } else {
        container.style.flexDirection = 'row';
        // KEY FIX: Center align items vertically for horizontal layouts
        container.style.alignItems = 'center';
    }

    // Override alignment if gravity is specified
    if (gravity === 'center') {
        container.style.alignItems = 'center';
        container.style.justifyContent = 'center';
    } else if (gravity === 'end' || gravity === 'right') {
        container.style.justifyContent = 'flex-end';
    } else if (gravity === 'start' || gravity === 'left') {
    ...
    if (icon) {
        ...
        // Handle both formats: 'bx bx-map' or just 'bx-map'
        const iconClass = icon.includes('bx') ? icon : `bx ${icon}`;
        iconElement.className = iconClass;
        iconElement.style.fontSize = safeDp(iconSize || textSize);
        iconElement.style.color = iconColor || textColor || 'var(--md-on-surface)';
        iconElement.style.flexShrink = '0';
        iconElement.style.display = 'flex';
        iconElement.style.alignItems = 'center';
        iconElement.style.justifyContent = 'center';
        // KEY FIX: Set line-height to 1 to prevent extra spacing
        iconElement.style.lineHeight = '1';

        // Apply spacing based on gravity
        if (iconGravity === 'start') {
            iconElement.style.marginRight = safeDp(iconSpacing);
        } else if (iconGravity === 'end') {
            iconElement.style.marginLeft = safeDp(iconSpacing);
        } else if (iconGravity === 'top') {
            iconElement.style.marginBottom = safeDp(iconSpacing);
        } else if (iconGravity === 'bottom') {
            iconElement.style.marginTop = safeDp(iconSpacing);
        }
    }

    // --- Content Wrapper for Expandable ---
    const contentWrapper = document.createElement('div');
    contentWrapper.style.display = 'flex';
    contentWrapper.style.flexDirection = 'column';
    contentWrapper.style.flex = '1';
    contentWrapper.style.minWidth = '0';
    // KEY FIX: Ensure wrapper doesn't add extra height
    contentWrapper.style.alignSelf = icon && (iconGravity === 'start' || iconGravity === 'end') ? 'center' : 'stretch';
    ...
    if (expandable) {
        applyCollapsedState();
        contentWrapper.appendChild(textView);
        ...
        const alignMap = {
            'start':  { alignSelf: 'flex-start', textAlign: 'left'   },
        ...
        const btnAlign = alignMap[seeMoreAlign] || alignMap['start'];

        seeMoreBtn.style.background   = 'none';
        seeMoreBtn.style.border       = 'none';
        seeMoreBtn.style.color        = seeMoreColor;
        seeMoreBtn.style.fontSize     = safeDp(textSize);
        seeMoreBtn.style.fontWeight   = '500';
        seeMoreBtn.style.padding      = '4px 0';
        seeMoreBtn.style.marginTop    = '4px';
        seeMoreBtn.style.display      = 'none';
        seeMoreBtn.style.alignSelf    = btnAlign.alignSelf;
        seeMoreBtn.style.textAlign    = btnAlign.textAlign;
        seeMoreBtn.style.fontFamily   = 'inherit';
    ...
    // Append icon and content based on gravity
    if (icon) {
        if (iconGravity === 'end' || iconGravity === 'bottom') {
    ...
    return {
        ...
        setIcon: (newIcon) => {
            if (iconElement && newIcon) {
                const iconClass = newIcon.includes('bx') ? newIcon : `bx ${newIcon}`;
                iconElement.className = iconClass;
            }
        },
   ...
   // ==========================================
// MaterialImage Component (Enhanced with Shimmer)
// ==========================================
...
const MaterialImage = (props = {}) => {
    const {
        ...
        shimmerBaseColor = 'rgba(0, 0, 0, 0.08)', // Base shimmer color
        shimmerHighlightColor = 'rgba(255, 255, 255, 1.0)', // Shimmer highlight
        shimmerAnimationDuration = '1.5s', // Shimmer animation speed
        onClick,
        onLoad,
        onError,
        onRetry,
    ...
    // Shimmer element for loading/error states
    const shimmer = document.createElement('div');
    shimmer.className = 'material-image-shimmer';
    applyStyles(shimmer, {
    ...
    shimmerOverlay.className = 'material-image-shimmer-overlay';
    applyStyles(shimmerOverlay, {
        position: 'absolute',
        top: '0',
        left: '-100%',
        width: '100%',
        ...
        background: `linear-gradient(90deg, 
            transparent 0%, 
            ${shimmerHighlightColor} 50%, 
            transparent 100%)`,
        animation: `shimmerSlide ${shimmerAnimationDuration} ease-in-out infinite`,
    });

    shimmer.appendChild(shimmerOverlay);
    container.appendChild(shimmer);

    // Add shimmer keyframe animation to document if not already present
    if (!document.getElementById('material-image-shimmer-keyframes')) {
        const style = document.createElement('style');
        style.id = 'material-image-shimmer-keyframes';
        style.textContent = `
    ...
    // Load high quality image for blur-up effect
    const loadHighQualityImage = (highQualitySrc) => {
        const tempImg = new Image();
        if (crossOrigin) tempImg.crossOrigin = crossOrigin;
        ...
        tempImg.addEventListener('load', () => {
            ...
            setTimeout(() => {
                ...
                img.style.opacity = opacity;
                shimmer.style.opacity = '0'; // Hide shimmer
                hasLoaded = true;
                if (onLoad) onLoad();
            }, 50);
        });
    ...
    const handleError = (error) => {
        ...
        // Keep showing shimmer on error (no error UI)
        shimmer.style.opacity = '1';
        img.style.opacity = '0';
    ...
    return {
        ...
        setShimmerColors: (baseColor, highlightColor) => {
            if (baseColor) {
                shimmer.style.background = baseColor;
            }
            if (highlightColor) {
                shimmerOverlay.style.background = `linear-gradient(90deg, 
                    transparent 0%, 
                    ${highlightColor} 50%, 
                    transparent 100%)`;
            }
...
// ==========================================
    // MaterialInput — Self-injecting styles
    // ==========================================
    (() => {
        if (document.getElementById('material-input-styles')) return; // already injected
        const style = document.createElement('style');
        style.id = 'material-input-styles';
        style.textContent = `
            /* --- Wrapper --- */
            .material-input-wrapper {
                position: relative;
            ...
            /* --- Label default (sitting inside the field) --- */
            .material-input-label {
                position: absolute;
                left: 12px;
                top: 50%;
                ...
                /* Prevent long hint text from overflowing into adjacent fields */
            ...
            /* --- Textarea label starts at the top, not vertically centred --- */
            .material-input-wrapper:has(textarea) .material-input-label {
                top: 16px;
                transform: none;
            }

            /* --- Floated state — label rises above the border line --- */
            .material-input-wrapper.material-input-floated .material-input-label {
                top: 0;
                transform: translateY(-50%);
                font-size: 12px;

                /* Sit on top of the border without the line cutting through the text */
                background-color: var(--md-input-bg, #fff);
                padding: 0 4px;

                /* Still clip if the label is very long even when floated */
            ...
            /* --- Shift label right when a leading icon is present --- */
            .material-input-has-leading .material-input-label {
                left: 40px;
    ...
    const MaterialInput = (props = {}) => {
        ...
        // Initial float state — textarea always floated so label never sits inside the text area
        if (value || isTextarea) {
        ...
        input.addEventListener('blur', () => {
            ...
            // Textarea label must NEVER go back inside — only unfloat regular inputs when empty
            if (!input.value && !isTextarea) {
        ...
        return {
            ...
            reset: () => {
                input.value = '';
                const classesToRemove = ['material-input-error', 'material-input-success'];
                // Only unfloat regular inputs on reset, textarea label stays at top
                if (!isTextarea) classesToRemove.push('material-input-floated');
...
/* ==============================================
   REQUIRED CSS additions / adjustments
   Add these rules to your stylesheet alongside
   your existing .material-input-* rules.
   ================================================

   FIX 1 — Long label clipping (belt-and-suspenders with the JS above)
   The JS already sets overflow/ellipsis inline, but this also prevents
   the label from painting outside the wrapper's stacking context:

   .material-input-wrapper {
       overflow: hidden;   ← ADD this if it isn't already there
   }

   If overflow:hidden breaks your focus ring or box-shadow, use:
   .material-input-wrapper {
       clip-path: inset(0 0 -8px 0);  ← lets bottom shadow/ring show
   }

   ------------------------------------------------
   FIX 2 — Textarea label always sits at the top

   Your current CSS likely positions the label with something like:
       top: 50%;
       transform: translateY(-50%);   ← vertically centres in a 1-line input

   For textareas that doesn't work because the field is tall.
   Add this override so the textarea label starts at the top edge:

   .material-input-wrapper.material-input-textarea .material-input-label {
       top: 14px;          ← match your normal input's top padding
       transform: none;    ← no vertical centering needed
   }

   The floated state (label moves above the field) should remain unchanged
   because the JS now keeps material-input-floated on the textarea at all
   times — the label simply stays in its floated/top position permanently.
   ============================================== */
...
const MaterialSpinner = (props = {}) => {
    ...
    // Attach dropdown to document.body instead of container.
    // This escapes any ancestor CSS transform (e.g. TabLayout's translateX on the
    // pages track), which would otherwise break position:fixed coordinate space.
  ...
  `;
    ...
    const closeDropdown = () => {
        ...
        // Keep custom color when a value is selected (floated label), reset otherwise
        if (color) labelEl.style.color = selectedValue ? color : '';
    ...
    // Close on outside click — dropdown is on document.body so check both
    ...
    const cleanup = () => {
        ...
        // Remove the body-level dropdown to avoid leaking DOM nodes
...
// ==========================================
    ...
    const MaterialButton = (props = {}) => {
        ...
        // Apply custom text color (overrides auto-calculated color from applyCustomColor)
        if (textColor) {
            button.style.color = textColor;
        }

        // Apply stroke (border) color and thickness
        if (strokeColor || strokeWidth) {
            const bColor = strokeColor || 'currentColor';
            const bWidth = strokeWidth ? `${strokeWidth}px` : '1px';
            button.style.border = `${bWidth} solid ${bColor}`;
        }
